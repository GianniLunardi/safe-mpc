...
class QuadrotorController:
    def __init__(self, model, solver_type='SQP_RTI'):
        self.model = model
        self.current_step = 0
        ...
        self.ocp.solver_options.nlp_solver_type = solver_type
...
ocp = QuadrotorController(model, solver_type='SQP')
mpc = QuadrotorController(model, solver_type='SQP_RTI')

ocp.set_cost(cost)
mpc.set_cost(cost)
ocp.build_controller(build = args['build'])
mpc.build_controller(build = args['build'])
ocp.resetHorizon(args['horizon'])
mpc.resetHorizon(args['horizon'])
...
print(f'Status: {status} , Check guess: {ocp.checkGuess()}')
if (status == 0 or status == 2) and ocp.checkGuess():
    print(f'Solver status {status}, x0 {x0}')
    print('IG SUCCESS')
    xg = copy.copy(ocp.x_temp)
    ug = copy.copy(ocp.u_temp)
    ...
    mpc.setGuess(xg, ug)
    mpc.reset_controller()

    for i in range(params.n_steps):
        u[i], sa_flag = mpc.step(x_sim[i])
        # mpc.ocp_solver.print_statistics()

        stats.append(mpc.getTime())
        x_sim[i+1], _ = model.integrate(x_sim[i], u[i])
